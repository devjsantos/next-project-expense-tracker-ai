...
import { Bell, ShieldAlert, Info, CheckCircle, RefreshCw, X } from 'lucide-react';
...
export default function NotificationCenter() {
  ...
  const ModalOverlay = () => {
    ...
    return createPortal(
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-slate-950/60 backdrop-blur-md">
        ...
        <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl p-10 border border-slate-100 dark:border-slate-800 animate-in zoom-in-95">
          <div className="flex items-center gap-5 mb-8">
            ...
            <div>
              <h4 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">{selected.title}</h4>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                Received: {new Date(selected.createdAt).toLocaleDateString()}
              </p>
            </div>
          </div>
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      <div className="max-h-[450px] overflow-y-auto p-4 space-y-3 custom-scroll">
            ...
            >
              <div className="flex gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                  n.read ? 'bg-slate-200 dark:bg-slate-700 text-slate-400' : 'bg-indigo-600 text-white'
                }`}>
